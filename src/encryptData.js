...
async function encryptData() {
  ...
  try {
    ...
    // Get ClientEncryption for manual encryption
    const clientEncryption = await getClientEncryption(client);

    // Access the source collection
    const dbName = process.env.MONGODB_DATABASE || 'myDb';
    ...
    for (const doc of documents) {
      ...
      // Create a new document with original and encrypted fields
      const encryptedDoc = {
        _id: doc._id,
        field_1: doc.field_1,
        field_2: doc.field_2,
    ...
    if (encryptedDocuments.length > 0) {
      ...
      console.log(`\nInserted ${encryptedDocuments.length} documents into encrypted_data collection`);
    }
